...
每个任务会保存入队当时的配置和 Cookie；入队后再用 `:set`、`:on` 等命令修改配置，只影响后续新任务。终端里可以用方向键上/下翻看历史输入，Tab 可以补齐交互命令和参数名；有多个匹配项时，终端会显示候选列表。历史默认保存到 `~/.media_downloader_history`；也可以用环境变量 `MEDIA_DOWNLOADER_HISTORY` 指定历史文件路径。

取消和退出都会停止当前任务，并把所有尚未开始的任务标记为已取消；区别是 `:cancel` 完成后仍保留交互提示符，可以继续添加新任务，而退出操作会同时关闭交互模式：

| 操作 | 当前任务 | 等待中的任务 | 操作后继续接收输入 |
| --- | --- | --- | --- |
| `:cancel` | 请求取消 | 全部取消 | 是 |
| `exit`、`quit`、`q` 或 `:quit` | 请求取消 | 全部取消 | 否 |
| Ctrl+C 或 EOF（通常是 Ctrl+D） | 请求取消 | 全部取消 | 否 |

取消是协作式的：下载、解析和后处理会在安全检查点停止；正在运行的 X 兼容转换会直接终止其 `ffmpeg` 子进程，再随主脚本退出。
...
X 转码启动 `ffmpeg` 时会禁用其标准输入，确保后台转换不会读取本应交给 `media>` 提示符的按键。在交互队列中执行时，输入 `:cancel`、退出命令、Ctrl+C 或 EOF 还会终止正在运行的 `ffmpeg` 子进程，不需要等待整个视频转换完成。
